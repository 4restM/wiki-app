...
const Search = () => {
  ...
  const handleSearch = async () => {
    ...
    const response = await fetch(`/api/search?query=${searchedItem}`);
    // const response = {
    //   batchcomplete: "",
    ...
    await saveSearchedItem(searchedItem);
  };

  const saveSearchedItem = async (item) => {
    try {
      const response = await fetch("/api/history", {
        method: "Post",
        headers: { "Content-Type": "applicatoin/json" },
        body: JSON.stringify({ searchedItem: item }),
      });
      if (!response.ok) throw new Error("Failed to save search");
    } catch (error) {
      console.error("error saving searched item", error);
    }
  };
